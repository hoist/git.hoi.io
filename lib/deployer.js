...
import {
  _mongoose, Organisation, Application, ExecutionLogEvent, Event
}
from '@hoist/model';
import {
  NotificationLogger,
...
class Deployer {
  ...
  getAgenda() {
    return this.agenda || (this.agenda = Promise.promisifyAll(new Agenda({
      db: {
    ...
    return this.getAgenda().cancelAsync({
      ...
      if (this.application.settings.live.schedules) {
        return Promise.all(_.map(this.application.settings.live.schedules, (schedule, key) => {
          var scheduleJob = Promise.promisifyAll(this.agenda.create('create:event2', {
            application: this.application._id,
            environment: 'live',
    ...
    return Promise.resolve()
      .then(() => {
        if (fs.existsSync(this.getSymlinkPathPath())) {
          var stat = fs.statSync(this.getSymlinkPathPath());
          if (stat.isDirectory()) {
            return fs.rmdirAsync(this.getSymlinkPathPath());
          } else {
