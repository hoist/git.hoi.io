...
GitActionListener.prototype = {
  ...
  push: function (push) {
    ...
    this.processSecurity(push, 'push').then(function (gitObject) {
  ...
  permissableMethod: function (username, password, method, gitObject) {
    ...
    return this.getUser(username, password).then(function (user) {
      ...
      return hoist.Applications.findOneAsync({
        name: application,
        _id: {
          $in: org.applications
        }
  ...
  getUser: function (organisation, application, username, password) {
    return hoist.Member.findOneAsync({
      emailAddresses: {
        $elemMatch: {
          address: username
        }
