  ...
  push: function (push) {
    ...
    return this.processSecurity(push, 'push')
      .then(function (gitObject) {
        ...
        var preRecieveHookFile = path.resolve(gitObject.cwd, './hooks/pre-receive');
        fs.lstat(preRecieveHookFile, function (err) {
          if (err && err.code === 'ENOENT') {
            logger.info('writing hook file');
            self.createHookFile(hookFile, preRecieveHookFile, next);
  ...
  getUser: function (username, password) {
    return User.findOneAsync({
      ...
      if (user && user.passwordHash && user.passwordHash.length > 0 && user.verifyPassword(password)) {
        ...
        logger.warn('user doesn\'t have a password');
