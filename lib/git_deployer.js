...
GitDeployer.prototype.clearOldDirectories = function (job) {
  ...
  var targetPath = path.join(config.get('Hoist.executor.root'), parts[parts.length - 2], parts[parts.length - 1].replace('.git', ''));
  return fs.readdirAsync(targetPath)
    .then(function (directories) {
      ...
      return BBPromise.all(_.map(directoriesToDelete, function deleteDirectory(dir) {
        return rimraf(path.join(targetPath, dir));
      }));
    });
};
...
GitDeployer.prototype.npm = function npm(job) {
  var gitRepoPath = job.path;
  var parts = gitRepoPath.split(path.sep);
  var rootPath = path.join(config.get('Hoist.executor.root'), parts[parts.length - 2], parts[parts.length - 1].replace('.git', ''));
  ...
  if (fs.existsSync(npmCachePath)) {
    ...
    promise = fs.copyAsync(path.join(rootPath, '.npmcache'), path.join(targetPath, 'node_modules'));
  }
  return promise.then(function () {
    ...
    return fs.copyAsync(npmModulesPath, npmCachePath);
  });
};
...
GitDeployer.prototype.loadHoistJson = function loadHoistJson(gitRepoPath, callback) {
