...
GitDeployer.prototype.deploy = function (job, log, write, done) {
  ...
  self.logToDeveloperConsole('Deploy started', job);
  ...
  return self.checkout(job)
    ...
    }).then(function completeLog() {
      var response = Array.prototype.slice(arguments);
      return self.logToDeveloperConsole('Deploy complete', job).then(function() {
        return response;
      });
...
GitDeployer.prototype.checkout = function checkout(job) {
  ...
  return mkdirp(targetPath).then(function () {
    ...
    var repo = BBPromise.promisifyAll(new GitRepo(gitRepoPath));
    repo.isRepoAsync().then(function (isRepo) {
      if (isRepo) {
        repo.branchesAsync().then(function (branches) {
          if (branches.length > 0) {
            job.lastCommitMessage = branches[0];
          }
        });
      }
    });
...
GitDeployer.prototype.logToDeveloperConsole = function log(message, job) {
  return this.getDetailsFromPath(job.path)
    ...
    .then(function(application) {
      var logEventObject = new ExecutionLogEvent({
        application: application._id,
        environment: 'live',
        type: 'DEPLOY',
        message: message
      });
      return logEventObject.saveAsync();
    }).then(function() {
      return job;
    }).catch(function(err) {
      logger.error(err);
    ...
    .then(function processSettings(application) {
      ...
      return this.loadHoistJson(job.path, job.log).bind(this)
        .then(function saveSettings(settingsFromJson) {
          ...
          //set commit message
          if (job.lastCommitMessage) {
            application.lastCommit.live = {
              message: job.lastCommitMessage.subject,
              sha1: job.lastCommitMessage.sha,
              user: job.lastCommitMessage.author.name + ' ' + job.lastCommitMessage.author.email
            };
          }
          //save application
